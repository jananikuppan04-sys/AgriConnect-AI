...
/* ------------------------------------------------------------------ */
/*  Source label helper                                                 */
/*  Handles both legacy string sources and new {filename, page} objects */
/* ------------------------------------------------------------------ */
function formatSources(sources) {
  if (!sources || sources.length === 0) return "";
  return sources
    .map((s) => {
      if (typeof s === "string") return s;          // legacy plain-string
      const label = s.filename ?? "unknown";
      return s.page != null ? `${label} — Page ${s.page}` : label;
    })
    .join(", ");
}
...
export default function Assistant() {
  ...
  async function sendMessage(userText) {
    ...
    try {
      ...
      // Append bot answer with optional sources and provider_used
      setMessages((prev) => [
        ...prev,
        {
          role: "bot",
          ...
          <div className="assist-messages" role="log" aria-live="polite">
            {messages.length === 0 ? (
              ...
              messages.map((msg, idx) => (
                <div key={idx} className={`msg-row ${msg.role === "user" ? "user" : "bot"} ${msg.provider_used === "unavailable" ? "warning" : ""}`}>
                  {/* Avatar */}
                  <div className={`msg-avatar ${msg.role === "user" ? "user-av" : "bot-av"}`}>
                    {msg.role === "user" ? "👤" : (msg.provider_used === "unavailable" ? "⚠" : "🌾")}
                  </div>

                  {/* Bubble + sources */}
                  <div className="msg-bubble-wrap">
                    <div className="msg-bubble">{msg.text}</div>
                    {msg.role === "bot" && msg.sources && msg.sources.length > 0 && msg.provider_used !== "unavailable" && (
